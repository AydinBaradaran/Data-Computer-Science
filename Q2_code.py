...
ride_predict = ridge_reg.fit(X_sub.reshape(-1,1),y).predict(X_sub.reshape(-1,1));
ride_predict2 = ridge_reg.fit(X_sub2,y).predict(X_sub2);
error_ridge = LA.norm(y-ride_predict2);
error_ridge = error_ridge**2;
print("without ridge regression error is ",error_ridge);    
